...
<%
    // ============================================================
    // [제조완료] 버튼 처리 (workOrderProgressMaking.jsp)
    // 재고 차감 없이 진행현황만 '승인요청'으로 변경 (실제 완료 처리는 승인 단계에서 이루어짐)
    // ============================================================
    ...
    try {
        ...
        String sql = "UPDATE work_order_requests SET progress_status = '승인요청' WHERE request_id = ?";
        pstmt = conn.prepareStatement(sql);
        pstmt.setInt(1, requestId);
        int affected = pstmt.executeUpdate();

        if (affected > 0) {
            out.print("{\"success\":true,\"message\":\"승인요청 되었습니다.\"}");
        } else {
