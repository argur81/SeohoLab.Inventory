...
<%
    ...
    try {
        ...
        // 진행현황을 '제조중'으로 변경
        String sql = "UPDATE work_order_requests SET progress_status = '제조중' WHERE request_id = ?";
        pstmt = conn.prepareStatement(sql);
        pstmt.setInt(1, requestId);
        int affected = pstmt.executeUpdate();

        if (affected > 0) {
            out.print("{\"success\":true,\"message\":\"제조가 시작되었습니다.\"}");
        } else {
        ...
        e.printStackTrace();
        out.print("{\"success\":false,\"message\":\"상태 변경 중 오류가 발생했습니다: " + e.getMessage().replace("\"", "'") + "\"}");
    } finally {
